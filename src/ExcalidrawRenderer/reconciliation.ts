...
import type { BroadcastedExcalidrawElement, ReconciledElements } from './constants';
import { arrayToMapWithIndex } from './constants';
import { AppState } from '@excalidraw/excalidraw/types/types';
const shouldDiscardRemoteElement = (localAppState: AppState, local: ExcalidrawElement | undefined, remote: BroadcastedExcalidrawElement): boolean => {
  if (
    local &&
...
export const reconcileElements = (
  ...
  for (const remoteElement of remoteElements) {
    ...
    // excalidraw version
    const parent = remoteElement[PRECEDING_ELEMENT_KEY] || remoteElements[remoteElementIdx - 1]?.id || null;
  ...
  const ret: readonly ExcalidrawElement[] = reconciledElements.filter((element) => !duplicates.has(element));
